...
function getProjectImagePath(slug, fileName) {
  return `${BASE_PATH}projects/${slug}/${fileName}`;
}

function getProjectUrl(slug) {
  return `${BASE_PATH}project.html?slug=${encodeURIComponent(slug)}`;
}

function getDataUrl() {
  return `${BASE_PATH}data/projects.json`;
}
...
function renderProjectCard(project) {
  ...
  return `
    <a class="project-card" href="${getProjectUrl(project.slug)}">
      <div class="project-card-media">
        ${mediaHtml}
      </div>
      <div class="project-card-body">
...
async function initHomePage() {
  ...
  try {
    const projects = await fetchJson(getDataUrl());
